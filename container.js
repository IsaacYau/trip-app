  ...
  capturePageSnapshot() {
    ...
    html2canvas(document.body, {
      ...
      .catch(error => {
        ...
        // Fallback: Create a dummy gradient canvas snapshot so WebGL can compile and run
        const dummyCanvas = document.createElement('canvas')
        dummyCanvas.width = 100
        dummyCanvas.height = 100
        const dummyCtx = dummyCanvas.getContext('2d')
        const gradient = dummyCtx.createLinearGradient(0, 0, 100, 100)
        gradient.addColorStop(0, '#070b19')
        gradient.addColorStop(1, '#111827')
        dummyCtx.fillStyle = gradient
        dummyCtx.fillRect(0, 0, 100, 100)
        
        Container.pageSnapshot = dummyCanvas
        Container.isCapturing = false
